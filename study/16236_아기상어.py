...
sys.stdin = open("input_data/16236_아기상어")
    ...
    while q:
        ...
        for _ in range(size):
            ...
            if board[x][y] != 0 and board[x][y] < shark:
                ...
                # 먹고 난 뒤, 현재 위치를 기준으로 다시 근처를 탐색해야 하기 때문에, # BFS queue 와 visited 를 초기화 해준다.
...
n = int(input())
board = [list(map(int , input().split())) for _ in range(n)] # 1. 초기 상어(자신)의 위치를 파악하고, 해당 자리는 판에서 비워둔다.
s_x, s_y = None, None
for i in range(n):
    for j in range(n):
        if board[i][j] == 9:
            s_x, s_y = i, j
            board[i][j] = 0 # 2. 시작점에서 BFS 진행
